...
2018/10 - 2019/1
- 职位：前端开发
- 主要技术：\`Vue\`、\`Vue Router\`、\`Vuex\`、\`jQuery\`微信小程序
- 工作内容：开发单页面 PC 端采购配货系统。主要重难点有组件的使用和切换、多个组件实例之间共享的状态的管理（Vuex）。 UI 组件自造轮子或使用 Element 。
开发购物商城微信小程序，主要重难点是用户结算页面的业务逻辑，还有数据缓存问题。开发商户后台系统，主要用jquery，重难点是业务逻辑，涉及数据特别多。
...
### 天气查询应用
- 关键词：\`nodeJs\`
- 源码链接：https://github.com/linian1226/weather
- 用法：打开终端-> npm install weather-gigi  -> weather +城市名称（本地可不输入) 展示当前城市的天气和温度

## 教育经历
### 湖北大学 |  大学本科
2014/9 - 2018/6
- 荣誉：两次校二奖学金、一次校三等奖学金（共三学年）
- 社团与其他：班级宣传委员，学院宣传部部长
`
    }
  },
  computed:{
